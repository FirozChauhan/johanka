...
type Stage =
  ...
  | "storing" // forwarding to StreamTape
  | "error";

/*
  Upload page. Files are forwarded to StreamTape (free storage) by the
  server at POST /api/upload. Thumbnails are generated by StreamTape itself
  once the video finishes processing — nothing to configure here. When a
  StreamTape folder id is configured server-side, uploads are placed into
  that folder.
*/
export default function UploadPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    // No credentials are sent — the server resolves StreamTape settings
    // itself (env + the settings table). The client never holds the config.
